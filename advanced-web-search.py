...
def clean_url(url: str) -> str:
    """Remove tracking/query parameters that break pages."""
    parsed = urlparse(url)
    clean = f"{parsed.scheme}://{parsed.netloc}{parsed.path}"
    return clean

# ---------------------------
# Fetch main content from a webpage
# ---------------------------
def fetch_webpage_text(url: str) -> str:
    try:
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
...
def html_search(query: str, max_results: int = 5) -> list:
    """
    Returns list of valid, cleaned URLs from DuckDuckGo HTML search page.
    """
    search_url = f"https://html.duckduckgo.com/html/?q={query.replace(' ', '+')}"
    try:
        headers = {"User-Agent": "Mozilla/5.0"}
...
# ---------------------------
# Main retrieval + intent analysis
# ---------------------------
def search_and_analyze(query: str, num_results: int = 5, chunk_size: int = 256, threshold: float = 0.0):
    urls = html_search(query, max_results=num_results)
    if not urls:
        return {"query": query, "merged_intents": [], "message": "No URLs found"}
    ...
    for url in urls:
        ...
        if content:
            ...
            source_map.extend([url] * len(chunks))

    if not combined_chunks:
        return {"query": query, "merged_intents": [], "message": "No content could be fetched"}

    # Run your multi-intent summarization on combined content
    full_text = ' '.join(combined_chunks)
    result = understand_multi_intent_summary(full_text, chunk_size=chunk_size, threshold=threshold)
    result['sources'] = list(set(source_map))  # Include all sources used
    return result
...
# ---------------------------
if __name__ == "__main__":
    user_query = "Compare Tesla and BYD electric cars"
    result = search_and_analyze(user_query, num_results=5)
